...
### 📋 ממצאים שדווחו אך לא תוקנו (לא התבקשו במפורש)
- `admin.html` — כפתור/toast עם `↩` (title="החזר לניסיון"), וסבב ניטרליות
  מגדרית מלא שמעולם לא בוצע שם (כפתורי ציווי בזכר כמו `שמור`/`רענן`/`הפעל`).
  עדיפות נמוכה: כלי פנימי לבעלים בלבד.
- `landing.html` — `תמשיך לקרוא` ו-`כדי שתמשיך לייצר ערך גם מחר` (פסקת "קצת
  עלינו") נשארו בזכר בלבד.
