...
> **חל איסור מוחלט להשתמש בסימני מקף רגיל (-) או חצים (→, ←, <, >) בטקסטים גלויים למשתמש
> או על גבי כפתורים**, ללא שום יוצא מן הכלל (כולל דף נחיתה, index, admin, toast, placeholder).
> - אם נדרש סימן הפרדה: השתמש ב-**•** (bullet) או **פסיק** בלבד.
> - מקף em/en (— ו–) מותרים כסימן פיסוק לשוני תקני בלבד (לא כחלק מ-"AI text pattern").
> - חצים ב-UI data (כמו referral flow directions) — להחליף ב-**•** או בניסוח מילולי.
