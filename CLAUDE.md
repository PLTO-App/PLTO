...
### 📌 הערות
- לא נבדק ויזואלית עם Playwright (`playwright-core` לא מותקן בסביבת הסשן הזו) —
  שינוי טקסט בלבד באותה מבנה HTML/CSS קיים (h1 + span.highlight + br), אין
  שינוי מבנה/עיצוב שדורש אימות חזותי.
