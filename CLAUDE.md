...
1. **הפניית ליד לבעל מקצוע חיצוני שבורה בפועל** — מיגרציה חיה מ-6/8/2026
   ...
   `OppBoard.publish()`. **הוחלט מול המשתמש (7/8) לתעד בלבד כרגע, לא לתקן** — לא ברור אם
   ההרחבה לכל התחומים הייתה מכוונת (סביר, יש constraint כפול ברמת DB שנראה מחושב) או
   תאונה, וטרם התקבלה החלטה על הכיוון (לתקן את ה-UI כך שיתאים לחסימה הרחבה, או לצמצם את
   החסימה בחזרה לעו"ד בלבד כמתועד במקור).
