...
## 🧪 בדיקה מקיפה שבועית — כלל קבוע לכל הסשנים (נקבע 17/7/2026)

> המשתמש ביקש בדיקת רגרסיה מקיפה **קבועה, לפחות פעם בשבוע**, על כל המערכת
> (`index.html`, `admin.html`, `landing.html`) עד שהמערכת תעבוד יציב בפועל
> ויהיו לקוחות קבועים. **הקצב עדיין ידני**: המשתמש מבקש את הבדיקה בעצמו כל
> פעם (לא Routine/cron אוטומטי כרגע) — כשהמערכת תתייצב עם לקוחות אמיתיים,
> הוא יבקש להעביר לריצה שבועית אוטומטית.

### מה כוללת הבדיקה (ולמה, לא רק "מה")
1. **Syntax** — `node qa/check_syntax.js index.html admin.html landing.html
   sign.html privacy-policy.html` (מפרק כל בלוקי ה-`<script>` בכל דף ובודק
   כל אחד בנפרד עם `vm.Script`, כולל הסרת הערות HTML קודם כדי לא להיתקע על
   `<script>` שמוזכר בתוך קומנט טקסטואלי, לא רק בקוד אמיתי).
2. **TDZ/סדר הגדרות מודולים** — נבדק דרך scope גלובלי אמיתי בעמוד (לא
   `window[name]`, שנותן false-negative על כל מודול שמוצהר כ-`const` בלי
   `window.X=` מפורש. ראה `qa/qa_index.js`).
3. **Supabase Advisors** (`mcp__Supabase__get_advisors`, security + performance)
   — כלי MCP, לא סקריפט מקומי.
4. **סקירת קומיטים אחרונים** מאז הבדיקה הקודמת (`git log`) — לוודא שכל תיקון
   אבטחה/DB שבוצע בקוד אכן חי בפועל ב-DB (GRANT/RLS/מיגרציות), לא רק שהקובץ
   השתנה.
5. **Playwright offline smoke** — `bash qa/run_all.sh` (מריץ את שלושת
   `qa/qa_*.js`: בודק שהדף נטען בלי page/console errors חריגים, מודולים
   מוגדרים, `escapeHtml()` חוסם XSS, אין גלילה אופקית ב-390px/1440px, אין
   אזכור מיתוג ישן). backend מדומה בזיכרון, בלי צורך ברשת/DB אמיתי. דורש
   `NODE_PATH=/opt/node22/lib/node_modules` (playwright גלובלי בסביבה).
6. **סריקת סגנון** (ניטרליות מגדרית, איסור חצים/מקפים) — עם regex תקין
   ל-unicode (`grep -nP '[\x{5D0}-\x{5EA}]+/[\x{5D0}-\x{5EA}]'` וכו'), לא
   `grep -oE` גולמי שנותן false-positives על עברית רב-בייטית.
7. **בדיקה חיה** מול plto.app בפועל — דרך `net.http_get` ב-Supabase (ה-sandbox
   חסום לרשת ישירה), + סטטוס הדיפלוי האחרון ב-GitHub Actions.

### 🔓 הרשאה קבועה: תיקון עצמאי של ממצאים מהבדיקה השבועית הזו בלבד
> אושר במפורש מול המשתמש (17/7/2026): כשהבדיקה השבועית הזו מוצאת באג/רגרסיה
> אמיתיים, **מותר לתקן ישירות (כולל commit + push לענף), בלי לעצור ולבקש
> אישור מראש** — ואז לעדכן את המשתמש בתמצית אחרי שהתיקון בוצע בפועל. זה
> **חריג ממוקד** לכלל הכללי (לעצור ולשאול לפני שינויים משמעותיים) — תקף
> ספציפית לממצאים שעלו **מתוך הבדיקה השבועית המתועדת כאן**, לא לכל שינוי
> יזום אחר. תיקון שדורש שינוי סכימה/מיגרציה משמעותי, נוגע בתשלומים, או לא
> חד-משמעי, עדיין עוצר ושואל קודם, כמו תמיד.

### קבצים
כל הסקריפטים נשמרים ב-`qa/` (`check_syntax.js`, `qa_index.js`, `qa_admin.js`,
`qa_landing.js`, `run_all.sh`, `README.md`). עדכון קוד האפליקציה לא דורש
עדכון הסקריפטים האלה אלא אם נוסף מודול/מסך שצריך כיסוי חדש.
...
### ✅ עדכון מאותו יום — הוחלט מול המשתמש, סקריפטים נשמרו בריפו
1. **הבדיקה השבועית הופכת קבועה** (ראה כלל "🧪 בדיקה מקיפה שבועית" בראש הקובץ).
   הסקריפטים הועברו מ-scratchpad זמני ל-`qa/` בריפו (`check_syntax.js` נכתב
   מחדש בסבב הזה, חזק יותר מהגרסה המקורית: מפרק כל בלוקי ה-script עם regex
   שמדמה בפועל את התנהגות ה-tokenizer של הדפדפן, כולל טיפול בהערת HTML
   שמזכירה `<script>` בטקסט חופשי ובעבר גרמה ל-false positive).
2. **הרשאה עצמאית לתיקון ממצאי הבדיקה השבועית** (ללא עצירה לאישור מראש, רק
   עדכון אחרי מעשה) — פרטים מלאים בכלל הקבוע בראש הקובץ.
3. **לא הוקם Routine אוטומטי** — המשתמש ביקש להישאר על בקשה ידנית כל שבוע,
   עד שיהיו לקוחות קבועים ומערכת יציבה בפועל, ואז יבקש לעבור לריצה אוטומטית.
