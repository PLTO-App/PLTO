...
### ✅ הפתרון
`@media (max-width: 480px)`: האייקון יורד לשורה נפרדת מעל הטקסט
(`.excel-title-row { flex-direction: column }`), כך שהטקסט מקבל את מלוא רוחב
המסך. עם הרוחב הנוסף מספיק גופן 21px (רק ~13% קטן מ-24px המקורי, ועדיין
גדול מטקסט הגוף) כדי שהכותרת תיכנס בדיוק ל-2 שורות מ-360px ומעלה. ב-320-340px
(מקרה קצה נדיר) נשארות 3 שורות, מקובל לפי אותו תקדים מהתיקון של כותרת הגיבור
בסשן 15/7 (א'). דסקטופ וטאבלט (481px+) לא נגעו בהם, נשארו `flex-direction: row`.

### 🔬 בדיקה
Playwright בכל הרוחבים 320-480px + השוואת דסקטופ (1440px, לאחר `scrollIntoView`
כדי לעקוף את אנימציית ה-`fade-up` שמשאירה את הרכיב ב-`opacity:0` עד גלילה) —
אין רגרסיה בפריסת הדסקטופ, אין גלילה אופקית חדשה (5px הקיים מראש מאנימציית
"המשולש הקדוש", לא קשור לתיקון הזה).
