...
### 🔒 שלושה רעיונות שסומנו מסוכנים, עם תנאי סף לפני בנייה (סומן 15/7/2026, עודכן 6/8/2026)
...
**3. תיווך לידים תמורת עמלה ע"י משתמש ללא רישיון תיווך** (רעיון חדש, הועלה 6/8/2026,
חלק מהרחבת "לוח ההזדמנויות" למישהו שאינו סוכן נדל"ן כלל)
> הרעיון: אדם פרטי ללא רישיון תיווך נרשם ל-PLTO, מאתר עסקת נדל"ן פוטנציאלית (מוכר או
> קונה), ומפנה אותה לסוכן נדל"ן מורשה שעובד באזור הרלוונטי, תמורת אחוז מעמלת הסוכן
> אם וכאשר העסקה נסגרת.
- **חשש משפטי מרכזי**: קבלת תשלום/עמלה עבור תיווך בעסקת מקרקעין ע"י מי שאינו בעל
  רישיון תיווך עלולה להיחשב עיסוק בתיווך מקרקעין ללא רישיון, לפי חוק המתווכים
  במקרקעין, תשנ"ו-1996 (החוק דורש רישיון תיווך לצורך קבלת דמי תיווך, ומגביל גם
  חלוקת עמלה עם מי שאינו מתווך מורשה). זה שונה וחמור יותר מהמקרה של עו"ד (שם ההגבלה
  היא כלל אתיקה מקצועית על עורך הדין עצמו, סעיף 11ב, ראה סשן 4/8/2026) — כאן מדובר
  בחשש לעיסוק בלתי חוקי של הצד הלא-מורשה עצמו, לא רק כלל התנהגות מקצועית של הסוכן.
- **חשיפה משולשת**: (א) המפנה הלא-מורשה, עלול להיחשב עוסק בתיווך ללא רישיון. (ב)
  הסוכן המורשה שמחלק עמלה עם לא-מורשה, עלול לפגוע ברישיון שלו/להיחשף לתביעה. (ג)
  PLTO עצמה, כמי שבנתה ושיווקה במפורש מנגנון שמעודד אנשים ללא רישיון "להרוויח סכומים
  משמעותיים" מתיווך, בפרט אם הפיצ'ר משווק לקהל הרחב ולא רק לסוכנים קיימים במערכת.
- **תנאי סף לפני כל תכנון/בנייה**: ייעוץ משפטי מפורש ומוקדם (לא רק סקירת קוד/מדיניות
  פנימית) מעורך דין המתמחה בדיני תיווך מקרקעין בישראל, שיבחן במפורש אם המנגנון
  המתואר (הפניה חד-פעמית + עמלה מותנית בהצלחה) נכנס לגדר "תיווך" לפי החוק, ואם כן
  באילו תנאים (אם בכלל) ניתן לבנות אותו בצורה חוקית (למשל: הגבלה למשתמשים שכבר
  מחזיקים רישיון תיווך תקף, גם אם אינם עובדים באזור/בתחום הספציפי של ההפניה).
- **לא נבנה ולא מתוכנן טכנית** עד שיתקבל אישור משפטי מפורש. אין לתכנן ארכיטקטורה/DB
  עבורו גם אם יעלה ביקוש ממשתמשים, בלי חזרה לסעיף הזה קודם.

> ⚠️ **עדכון 6/8/2026, אחרי סשן לוח הנכסים והקונים: הפער הזה רחב יותר משנראה במקור.**
> תחום "סוכן נדל"ן" באונבורדינג הוא **בחירה עצמית לגמרי** מתוך רשימה, בלי שום שדה
> מספר רישיון ובלי שום אימות מול מרשם (אומת: 0 אזכורים של "רישיון"/"license" בכל
> `index.html`). המשמעות: החשש שתואר למעלה **כבר קיים בפועל בפיצ'רים ששולחו מזמן**,
> לא רק ברעיון עתידי, כי `lead_referrals` (עמלת הפניה בין קולגות) ו-`partner_
> opportunities` (לוח ההזדמנויות, גם עם עמלה) פתוחים לכל מי שסימן "סוכן נדל"ן",
> בלי שום בדיקה שהוא באמת מורשה. גם לוח הנכסים והקונים החדש (`market_board_posts`),
> שנבנה במפורש **בלי** עמלה שהמערכת מתווכת (כדי להימנע מהסיכון של "PLTO מחלקת עמלה
> עם לא-מורשה"), לא פותר את השאלה היסודית יותר: גם בלי תיווך כספי של הפלטפורמה,
> מי שאין לו רישיון תיווך בכלל עלול להיחשב עוסק בתיווך שלא כדין, רק על בסיס
> הפעילות עצמה (תיאום מכירה/רכישה תמורת שכר מהלקוח שלו), ללא קשר למה שעובר דרכנו.
> **הוחלט מפורשות עם המשתמש**: לא לבנות שום מנגנון אימות רישיון כרגע, כי אין עדיין
> משתמשים אמיתיים שמבצעים הפניות בפועל, לא נחוץ עד שיהיה נפח משתמשים אמיתי. לתעד
> ולעצור. כשתגיע השאלה שוב (נפח משתמשים אמיתי בתחום סוכן נדל"ן): לבדוק אם מערכות
> דומות דורשות הוכחת רישיון (מספר רישיון מול מרשם המתווכים הרשמי, ו/או העלאת צילום
> תעודה לאימות ידני), ולשקול קודם את פיצ'ר "קיר תעודות והמלצות" (ראה טבלת רודמאפ
> למטה) כצעד ביניים לא-מחייב, **לא** כתחליף לאימות אמיתי אם וכשיוחלט לבנות שער
> גישה לפיצ'רי עמלה/הפניה.

> 🔴 **עדכון 6/8/2026 (המשך אותו סשן) — עמלה הוסרה בפועל מלולאת ההפניות ומלוח
> ההזדמנויות, לא רק תועדה כסיכון.** הוחלט מפורשות עם המשתמש: עד לייעוץ משפטי
> אמיתי, ולפחות עד נפח משתמשים משמעותי (המשתמש ציין כ-50 משתמשים מרחבי הארץ
> כסדר גודל גס, לא סף מדויק), הפיצ'ר ממשיך להתקיים אבל **תמיד ללא עמלה**, בלי
> יוצא מן הכלל לאף תחום (בעבר העמלה הייתה חסומה רק לעו"ד, ראה מיגרציה 111, סעיף
> 11ב). מיגרציה `116_disable_referral_commission_pending_licensing.sql` (הוחלה
> על ה-DB החי): הרחיבה את החסימה מ-`commission_not_allowed_lawyer` (רק עו"ד)
> ל-`commission_not_allowed` (כל תחום, כל הזמן), גם ב-`_create_lead_referral_core`
> וגם ב-`publish_opportunity`, וגם ב-CHECK constraints ברמת הטבלה (הגנה כפולה,
> אומתה: ניסיון INSERT ישיר עוקף-RPC עם עמלה נכשל גם הוא). ניקתה גם 2 שורות
> בדיקה ישנות מסשני פיתוח קודמים (על חשבונות הבדיקה של הבעלים בלבד, לא לקוח
> אמיתי) שהיו נכשלות מול ה-CHECK constraint החדש.
>
> **מה עדיין עובד**: הפניה פשוטה בין קולגות בשלושת התחומים (בלי עמלה, בלי
> חתימה) ולוח ההזדמנויות (בלי עמלה) ממשיכים לתפקד במלואם. **מה הפך רדום**
> (לא נמחק, נשאר לחזרה עתידית קלה): כל מנגנון ההסכם/חתימה הדיגיטלית
> (`referral_agreements`, `sign_referral_agreement`, `sign.html`) — לא ייווצר
> אף שורת עמלה חדשה יותר, אז אין יותר מה לחתום עליו. **הפניה לבעל מקצוע חיצוני
> ללא חשבון PLTO** ("🔨 בעל מקצוע אחר", שיפוצניק/קבלן) **הוסרה מהתפריט לגמרי**,
> לא רק העמלה שלה, כי כל הזרימה שלה בנויה סביב עמלה+חתימה (התמריץ לחתום על
> הסכם ולקבל פרטי ליד), ובלי עמלה אין לה תכלית קוהרנטית כמו שהיא בנויה היום.
> זה סיבה שונה מהחשש המשפטי על רישיון תיווך (לא כל תחום חיצוני קשור לתיווך
> נדל"ן בכלל), מתועד בנפרד כדי שלא יתבלבלו בעתיד.
>
> **הטריגר לבדוק שוב**: לא זמן קלנדרי, אלא נפח משתמשים אמיתיים (הערכה גסה:
> ~50), ולפני שמחזירים עמלה בפועל, ייעוץ משפטי מפורש הוא תנאי סף בנוסף לנפח,
> לא תחליף לו.
...
| כל התחומים | **קיר תעודות והמלצות** — העלאת תעודות/הסמכות/המלצות לקוחות, לשיתוף עם לידים כתוכן אמון (לא כאימות רישיון!) | 💡 רעיון, הועלה 6/8/2026 (ראה גם הערת "עדכון 6/8/2026" בסעיף רעיון #3 המסוכן למעלה, על פער אימות רישיון תיווך). טרם תוכנן טכנית |
...
## מה בוצע — סשן 6/8/2026 (ב') — לוח נכסים וקונים (Market Board), רק סוכן נדל"ן

> ענף: `claude/bmby-competitor-analysis-rb4lkc`. המשך ישיר לניתוח מתחרה bmby (MLS
> `4Expert`) מאותו סשן. תוכנן ב-Plan Mode. הרעיון עלה מהמשתמש: MLS מלא רלוונטי בעיקר
> למי שמחזיק נכס בבלעדיות, אבל סוכן שמייצג קונה יכול לפרסם "יש לי קונה שמחפש X באזור Y
> בתקציב Z" בלי לחשוף את הליד, וכך ליצור שת"פ עם סוכן מוכרים. **אין עמלה משותפת
> שהמערכת מנהלת** — כל סוכן כבר סגר בנפרד עם הצד שלו (המוכר/הקונה), בדיוק כמו הנוהג
> הענפי הרגיל, ולכן זה שונה מהותית מ"לוח ההזדמנויות"/"לולאת ההפניות" הקיימים ופשוט
> משמעותית מהם (בלי הסכם/חתימה/consent).

### ✅ הושלם
1. **מיגרציה `115_property_matching_board.sql`** (הוחלה על ה-DB החי) — טבלת
   `market_board_posts` (RLS מופעל, בלי policies, גישה רק דרך RPCs, דפוס זהה ל-
   `lead_referrals`/`partner_opportunities`): `post_type` (`listing`/`buyer_need`),
   אזור, עיר, סוג נכס, טווח מחיר/תקציב, חדרים, תיאור, וקישור אופציונלי לליד/נכס
   קיים של הטננט עצמו (לעולם לא נחשף לצד השני). RPCs: `publish_market_post` (דורש
   תחום `realestate`, rate limit 5/יום, שואב ברירות מחדל מליד/נכס מקור אם סופק),
   `list_market_board` (מחזיר זהות+טלפון המפרסם **מיד**, בלי שלב אנונימיות/מועמדות
   כמו ב-`list_open_opportunities`, זו ההחלטה שאושרה מול המשתמש), `list_my_market_posts`,
   `close_market_post`.
2. **באג אמיתי שנתפס בבדיקה**: `list_market_board` (RETURNS TABLE עם עמודה בשם
   `id`) יצר התנגשות שם משתנה עם `tenants.id` בבדיקת התחום (`WHERE id = v_tenant_id`
   היה מפורש כעמודת הפלט, לא כעמודת הטבלה) — כל קריאה הייתה נכשלת. תוקן ע"י כינוי
   מפורש לטבלה (`tenants t2 WHERE t2.id = ...`).
3. **`index.html`** — תת-טאב חדש בתוך `#ptab-content-board` הקיים (מתג "🤝 שיתופי
   פעולה"/"🏘 נכסים וקונים", לפי החלטת המשתמש למקם בתוך הטאב הקיים ולא טאב עליון
   נפרד), מודול `MarketBoard` חדש (מבנה מבוסס `OppBoard` הקיים), מודל פרסום משותף
   לשני סוגי הפוסט (`modal-publish-market-post`) עם מילוי אופציונלי מנכס/ליד קיים
   של הטננט. מתג "נכסים וקונים" מוסתר לגמרי לתחומים שאינם סוכן נדל"ן (`OppBoard.render()`
   מטפל בזה, כי הוא כבר נקרא בכל פתיחת הטאב).
4. **אבטחה/UX**: כרטיסי הלוח מעבירים רק `id` הפוסט ל-`onclick` (לא JSON גולמי
   בתוך attribute, כדי לא לסכן שבירת HTML על תווים כמו `&`), יצירת קשר דרך קישור
   `wa.me` עם הטלפון שהוחזר מה-RPC, `escapeHtml()` על כל שדה דינמי (תיאור/עיר/שם
   מפרסם), placeholder "מ-"/"עד" הוחלף ל"מינימום"/"מקסימום" (איסור מקף בטקסט UI).

### 🔬 בדיקה
נבדק ישירות מול ה-DB החי בטרנזקציית ROLLBACK (2 טננטי `realestate` אמיתיים + טננט
`other`): פרסום ע"י טננט נדל"ן מצליח; הלוח לא מציג את הפוסט של הטננט עצמו לעצמו;
טננט אחר רואה את הפוסט עם שם+טלפון המפרסם; `rate limit` נתפס בפוסט השישי; טננט
`other` נחסם גם בפרסום וגם בקריאה עם `vertical_not_supported`; סגירת פוסט עובדת
רק לבעלים. `node --check` על בלוק ה-`<script>` הראשי עבר נקי. לא הופעל Agent tool
(כולל `live-browser-qa`) בשלב הזה, לפי הכלל הקבוע שדורש אישור מפורש מראש.

### 📋 הערות
- לא נבנה מנגנון "reveal" חוצה-טננטים לזהות הליד (הקונה) — ההחלטה שהליד "נחשף"
  קורית מחוץ למערכת (סוכן הקונים מראה תמונות ללקוח שלו, מקבל אישור לסיור), אין
  צורך במנגנון consent נוסף כמו ב-`client_consents`.
- אין ספירת "עניין"/מספר צפיות על פוסט (בשונה מ-`applications_count` ב-OppBoard) —
  פושט את המנגנון, קשר נוצר ישירות בוואטסאפ בלי שלב ביניים במערכת.

---

## מה בוצע — סשן 6/8/2026 (ג') — הסרת עמלה מלולאת ההפניות ומלוח ההזדמנויות

> ענף: `claude/bmby-competitor-analysis-rb4lkc`. המשך ישיר לדיון על פער אימות
> רישיון תיווך (סשן ב' מעלה). הוחלט מול המשתמש: לא להשבית את כל פיצ'ר ההפניות,
> רק את שדה העמלה, עד ייעוץ משפטי + נפח משתמשים אמיתי (~50, לא סף מדויק). ראה
> תיעוד מלא, כולל הנימוק המשפטי, ב"עדכון 6/8/2026 (המשך אותו סשן)" תחת רעיון #3
> המסוכן למעלה.

### ✅ הושלם
1. **מיגרציה `116_disable_referral_commission_pending_licensing.sql`** (הוחלה
   על ה-DB החי) — הרחיבה את חסימת העמלה שהייתה קיימת רק לעו"ד (מיגרציה 111) לכל
   התחומים תמיד, ב-`_create_lead_referral_core`, ב-`publish_opportunity`,
   ובCHECK constraints ברמת הטבלה על `lead_referrals`/`partner_opportunities`
   (`commission_type = 'none'` תמיד). ניקתה גם 2 שורות עמלה ישנות מבדיקות
   פיתוח קודמות (חשבונות הבעלים בלבד).
2. **`index.html`** — הוסתרו לצמיתות (ב-CSS, לא נמחקו מה-DOM כדי שקל להחזיר
   בעתיד) שני בלוקי טופס העמלה (`lref-commission-wrap` במודל "הפנה לקולגה",
   `opp-commission-wrap` במודל "פרסום הזדמנות") + הערות ה"עו"ד" הישנות שכבר לא
   מדויקות. `LeadReferral.create()`/`OppBoard.publish()` שולחים תמיד
   `commission_type:'none'`. הוסרה לגמרי אפשרות "🔨 בעל מקצוע אחר" מתפריט
   ההפניה (בנויה כולה סביב עמלה+חתימה, ללא תכלית בלעדיה). עודכנו טקסטי הבאנרים
   (המשולש הקדוש, לוח ההזדמנויות, מדריך הדרכה) שהזכירו עמלה/עו"ד באופן שכבר
   לא מדויק.

### 🔬 בדיקה
נבדק ישירות מול ה-DB החי בטרנזקציית ROLLBACK: ניסיון הפניה עם עמלה לתחום סוכן
נדל"ן נכשל, ניסיון הפניה עם עמלה לתחום עיצוב פנים נכשל (לפני הסבב הזה זה היה
מותר, רק עו"ד היה חסום), הפניה בלי עמלה מצליחה, פרסום הזדמנות עם עמלה נכשל,
פרסום בלי עמלה מצליח, וניסיון INSERT ישיר שעוקף את ה-RPC עם עמלה נחסם ע"י
ה-CHECK constraint. `node --check` על בלוק ה-`<script>` הראשי עבר נקי.

### 📋 לא נגעתי (במפורש, נשאר רדום לא נמחק)
`referral_agreements`, `sign_referral_agreement`/`sign_referral_agreement_anon`,
`get_referral_agreement`/`get_referral_agreement_anon`, ו-`sign.html` נשארים
בקוד כפי שהם, פשוט לא ייווצר יותר שום referral עם עמלה שיפעיל אותם. `client_
consents` (הסכמת פרטיות לקוח) לא קשור לכסף ונשאר פעיל במלואו ללא שינוי.
