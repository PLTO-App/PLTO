-- Migration 118: Reconcile repo/DB drift — Property/Buyer Matching Board
--
-- DRIFT NOTE: a migration named "property_matching_board" (2026-08-06,
-- version 20260806122557) is already live in this project, applied directly
-- against the database with no matching file in this repo. Found during a
-- CLAUDE.md accuracy review that compared live schema/functions against the
-- repository. Reproduced below byte-for-byte (pulled from
-- supabase_migrations.schema_migrations.statements, the CLI's own record of
-- what actually ran) purely to close the repo/DB drift — no behavior change.
-- Chronologically this migration predates 115_lead_tracker.sql /
-- 116_lead_tracker_messaging.sql on the live database (12:25 vs 15:51 on the
-- same day), but is filed after them here since 115-117 were already taken
-- locally by the time this drift was discovered.
--
