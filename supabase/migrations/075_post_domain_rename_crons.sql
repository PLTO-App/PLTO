-- PLTO — Migration 075: Rename cron jobs from liders-* to plto-*
--
-- Run this AFTER plto.app is live and verified.
-- Safe to run multiple times (idempotent).

-- ── 1. support daily digest ────────────────────────────────────────────────
DO $$ BEGIN PERFORM cron.unschedule('liders-support-daily-digest'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
...
-- ── 2. leads daily digest ──────────────────────────────────────────────────
DO $$ BEGIN PERFORM cron.unschedule('liders-leads-daily-digest'); EXCEPTION WHEN OTHERS THEN NULL; END $$;

SELECT cron.schedule(
  'plto-leads-daily-digest',
  '0 15 * * *',
  $$SELECT public.send_daily_lead_digest()$$
);

-- ── 3. CRO weekly digest ───────────────────────────────────────────────────
DO $$ BEGIN PERFORM cron.unschedule('liders-cro-weekly-digest'); EXCEPTION WHEN OTHERS THEN NULL; END $$;

SELECT cron.schedule(
  'plto-cro-weekly-digest',
  '0 8 * * 1',
