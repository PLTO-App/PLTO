-- Migration 119: Reconcile repo/DB drift — disable referral/opportunity
-- commission entirely (temporary)
--
-- DRIFT NOTE: a migration named "disable_referral_commission_pending_licensing"
-- (2026-08-06, version 20260806132530) is already live in this project,
-- applied directly against the database with no matching file in this repo.
-- Found during a CLAUDE.md accuracy review that compared the live
-- `_create_lead_referral_core()` definition against repo migrations and the
-- CLAUDE.md "Referral commission" section. Reproduced below byte-for-byte
-- (pulled from supabase_migrations.schema_migrations.statements, the CLI's
-- own record of what actually ran) purely to close the repo/DB drift — no
-- behavior change. Chronologically this migration ran right after
-- 118_property_matching_board.sql on the live database (12:25 vs 13:25 on
-- the same day), both ahead of 115_lead_tracker.sql (15:51); filed after
-- 117 here since 115-117 were already taken locally by the time this drift
-- was discovered.
--
...
-- way to confirm today. See CLAUDE.md, "Legal, Privacy & Liability Gate" and
-- the follow-up decision to disable commission until (a) legal counsel is
...
--
-- Verified live at the time this file was written: 0 lead_referrals rows and
-- 0 referral_agreements rows have ever had a non-'none' commission_type —
-- the commission-with-signature path was never actually used in production
